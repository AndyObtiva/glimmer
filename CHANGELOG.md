...
- Have `ObservableHash` call `notify_observers(key)` upon performing mutation operation: `delete`, `delete_if`, `filter!`, `select!`, `keep_if`, `reject!`, `merge!`, `replace`, `shift`, `transform_keys!`
