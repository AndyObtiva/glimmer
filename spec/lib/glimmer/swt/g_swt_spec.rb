  ...
  describe Glimmer::SWT::GSWT do
    describe '.constant' do
      ...
      it 'returns SWT constant value for symbol representing an extra SWT constant added in Glimmer for convenience' do
        expect(Glimmer::SWT::GSWT.constant(:no_resize)).to eq(Glimmer::SWT::GSWT[:shell_trim] & (~Glimmer::SWT::GSWT[:resize]) & (~Glimmer::SWT::GSWT[:max]))
      end
    ...
    describe '.[]' do
      ...
      context 'bad arguments' do
        it 'displays a friendly error message' do
          expect {Glimmer::SWT::GSWT[:bold, :beautiful]}.to raise_error("beautiful is an invalid SWT style! Please choose a style from org.eclipse.swt.SWT class constants.")
        end
      end
    end
  end
