...
Note: if you are using Glimmer manually, to make the `glimmer package` command available, you must add the following line to your application `Rakefile` (automatically done for you if you had scaffolded an app or gem first with `glimmer scaffold[AppName]` or `glimmer scaffold:gem:customshell[GemName]`):
