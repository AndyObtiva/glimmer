...
You may choose to generate a specific type of packaging by passing the `[type]` option (available Mac types: `dmg`, `pkg`, and `image` [meaning `app` without setup program] / available Windows types: `msi`, `exe`, and `image` [meaning `app` without setup program])
