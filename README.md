...
Glimmer simplifies the process of native-executable packaging and distribution on Mac and Windows via the `glimmer package` command (which mainly uses [Warbler](https://github.com/jruby/warbler) and [javapackager](https://docs.oracle.com/javase/8/docs/technotes/tools/unix/javapackager.html) behind the scenes):
...
You may choose to generate a specific type of packaging by passing the `[type]` option. For example, this generates an MSI setup file on Windows:
...
- Available Windows types are `msi`, `exe`, and `image` (image means a Windows `app` directory without a setup program)
