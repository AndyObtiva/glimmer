...
It works out of the box for any application generated by [Glimmer Scaffolding](#scaffolding). [CORRECTION: Windows requires an additional step to handle jar-dependencies. One must also run this command: `lock_jars --vendor-dir vendor` and add a gemspec that puts 'vendor' directory in `require_paths` first before 'lib']
